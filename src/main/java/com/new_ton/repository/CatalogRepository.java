...
public interface CatalogRepository extends JpaRepository<CatalogEntity, Integer> {
    ...
    @Query("SELECT new com.new_ton.domain.dto.technologistdto.CatalogDtoByLeftTable(cat.idpr, cat.datecr, cat.brend, cat.nameprod, cat.percent, cat.mass, cat.tempprodmin, cat.tempprodmax)  FROM CatalogEntity cat  WHERE LOWER(cat.nameprod) LIKE LOWER(CONCAT('%', ?1,'%'))")
    Page<CatalogDtoByLeftTable> findByNameProdWithPagination(String nameProd, Pageable pageable);

    @Query("select new com.new_ton.domain.dto.technologistdto.CatalogDtoSelectedRow(cat.datecr, cat.brend, cat.nameprod, cat.mass, cat.percent, cat.tempprodmin, cat.tempprodmax) from CatalogEntity cat where cat.idpr =?1")
