...
public interface CatalogRepository extends JpaRepository<CatalogEntity, Integer> {
    ...
    @Query("SELECT new com.new_ton.domain.dto.CatalogDtoByLeftTable(cat.idpr, cat.brend, cat.nameprod)  FROM CatalogEntity cat  WHERE LOWER(cat.nameprod) LIKE LOWER(CONCAT('%', ?1,'%'))")
    Page<CatalogDtoByLeftTable> findByNameProdWithPagination(String nameProd, Pageable pageable);

    @Query("select new com.new_ton.domain.dto.CatalogDtoSelectedRow(cat.datecr, cat.brend, cat.nameprod, cat.mass) from CatalogEntity cat where cat.idpr =?1")
