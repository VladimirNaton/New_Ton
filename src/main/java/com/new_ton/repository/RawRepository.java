...
public interface RawRepository extends JpaRepository<RawEntity, Integer> {
    @Query("select re from RawEntity re where re.idpr = ?1 and re.code <> 3 and re.code <> 4 and re.code <> 5 and re.code <> 8")
    List<RawEntity> findAllByIdProd(int idProd);

    @Query("select re from RawEntity re where re.idpr = ?1 and re.code = 8 and re.stage = 1")
    Optional<RawEntity> findCode8Stage1(int idProd);

    @Query("select re from RawEntity re where re.idpr = ?1 and re.code = 8 and re.stage = 2")
