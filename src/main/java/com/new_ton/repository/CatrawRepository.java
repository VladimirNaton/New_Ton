...
import com.new_ton.domain.dto.EditeRecipeComponentTableDto;
...
public interface CatrawRepository extends JpaRepository<CatrawEntity, Integer> {
    @Query("select new com.new_ton.domain.dto.EditeRecipeComponentTableDto(cat.id , cat.date, cat.nameraw) from CatrawEntity cat where cat.code =?1")
    Page<EditeRecipeComponentTableDto> getDataForEditeRecipeComponentTable(Integer code, Pageable pageable);

    @Query("select new com.new_ton.domain.dto.EditeRecipeComponentTableDto(cat.id , cat.date, cat.nameraw) from CatrawEntity cat where cat.code =?1 and LOWER(cat.nameraw) LIKE LOWER(CONCAT('%', ?2,'%') )")
